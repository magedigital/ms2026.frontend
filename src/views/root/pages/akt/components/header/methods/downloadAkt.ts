...
const downloadAkt: I['downloadAkt'] = async function () {
    ...
    await this.asyncSetState({ loadingKey: 'download' });

    try {
        const { pdf } = await anketRequests.getAkt({ id });
        downloadPDF(pdf, id!);
    } catch (er) {
        const error = er as ResponseErrorT;
        await this.asyncSetState({ error: error?.error });
    }

    await this.asyncSetState({ loadingKey: undefined });
};

export default downloadAkt;
